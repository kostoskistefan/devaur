#ifndef ERROR_DEFINITIONS_H
#define ERROR_DEFINITIONS_H

#define CLONE_ERROR "Cannot clone AUR repository"
#define REMOTE_ERROR "Cannot find remote of AUR repository"
#define FETCH_ERROR "Cannot fetch AUR repository. Try cleaning your $HOME/.cache directory"
#define REMOVE_ERROR "Could not remove package"
#define INSTALL_ERROR "There was a problem installing the package"
#define CLONING_IN_PROGRESS "Cloning repository..."
#define INSTALLATION_IN_PROGRESS "Installing package..."
#define CLEAN_CACHE_PROMPT "Are you sure you want to clean the cache? (y/n) "
#define EXIT_MESSAGE "Exiting..."

#endif
